...
for (const kind of ['control', 'source']) {
  for (const file of readdirSync(join(types, kind))) {
    ...
    // Keyed by surface as well as name. <Toggle> is a button on the dashboard and
    // the thing being toggled on air -- two components, two entry points, and the
    // whole reason those entry points are separate. A map keyed by name alone would
    // silently render one of them twice.
    for (const [, name] of source.matchAll(/export declare function (\w+)\s*\(/g)) files.set(`${kind}/${name}`, source)
  }
}

/** Where a reader types to get it. The import line is half the answer once a name is on both sides. */
const IMPORT = { control: '@single-studio/core/control', source: '@single-studio/core/source' }

function render(kind, component) {
  const source = files.get(`${kind}/${component}`)
  const anchor = slug(kind, component)

  if (!source) return `<a id="${anchor}"></a>\n\n### ${component}\n\n_Not found in dist/types — was it renamed?_\n`
  ...
  // An explicit anchor, because the heading alone is no longer unique: GitHub would
  // give the second <Toggle> `#toggle-1`, which is a link that depends on the order
  // components happen to be listed in.
  const lines = [`<a id="${anchor}"></a>`, '', `### ${component}`, '', '---', '', `\`import { ${component} } from '${IMPORT[kind]}'\``, '', summary]
...
const slug = (kind, name) => `${kind}-${name.toLowerCase()}`

const link = (kind, name) => `[\`${name}\`](#${slug(kind, name)})`
const dash = (name) => link('control', name)
const air = (name) => link('source', name)
...
\`name\` row says which. The two that act on several values at once, ${dash('ResetButton')} and
${dash('SwapButton')}, take \`paths\` for the rare case of reaching outside \`variables\`.
...
which is an ordinary React component — put controls in a ${dash('Panel')} and it arranges
...
${AUTHORED.control.map((name) => render('control', name)).join('\n')}
## Source

What goes on air. Each of these lives in a graphic under \`src/sources/\`, one file
per OBS browser source, wrapped in a ${air('Scene')}. They read the same paths the
...
${AUTHORED.source.map((name) => `- ${air(name)}`).join('\n')}

${AUTHORED.source.map((name) => render('source', name)).join('\n')}`
